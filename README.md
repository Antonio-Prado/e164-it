...
### Bindings
- `DB` — D1 database binding (API keys and per-key limits)
- `RL` — KV namespace binding (rate limiting counters)

### Environment variables / secrets
- `API_KEY_SALT` — required; used to hash API keys server-side
- `ADMIN_TOKEN` — required; used to authorize admin endpoints
- `HASH_SECRET` — optional; required only if hashing is enabled

### Database schema note
The repository includes `migrations/0001_init_api_keys.sql`. The current API expects two additional columns:

- `max_batch_rows`
- `max_batch_bytes`

If your database was created from the provided migration only, add a follow-up migration similar to:

```sql
ALTER TABLE api_keys ADD COLUMN max_batch_rows INTEGER NOT NULL DEFAULT 5000;
ALTER TABLE api_keys ADD COLUMN max_batch_bytes INTEGER NOT NULL DEFAULT 1048576;
```
...
`package.json` declares the **ISC** license. Add a `LICENSE` file if you want the license text to be explicit in the repository.
